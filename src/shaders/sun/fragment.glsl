...
uniform float uTime;  // New uniform for time-based effect
...
float fbm(vec4 p) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    p.w += 100.0;  // Adding to w for animation (time)
    amp *= 0.9;  // Decrease amplitude for each iteration
    scale *= 2.0;  // Increase scale for fractal detail
  }

  return sum;  // Return the sum of noise
}
...
void main() {
  ...
  // Normalize the normal for lighting calculations (unused in current code, but can be added later)
  vec3 normal = normalize(vNormal);

  // Sample the sun texture to get the sunColor
  vec3 sunColor = texture2D(uSunTexture, vUv).rgb;

  // Use the noisy value to modulate the sun color
  vec3 color = sunColor * (0.5 + 0.5 * noisy);  // Scale noisy to [0.0, 1.0]

  // Optionally apply some effect (e.g., darken or brighten)
  color = mix(sunColor, color, 0.8);  // Blend sunColor with noise-based color

  // Final color output
  gl_FragColor = vec4(color, 1.0);  // Set the final color

  // Final tone mapping and color space adjustments
